...
const OUTPUT_DIR = path.resolve(__dirname, '../static/ygo/thumbs')
const FAILED_IDS_PATH = path.resolve(
  __dirname,
  '../static/ygo/failed-thumb-art.json'
)
...
function getThumbUrl(card) {
  ...
  return image.image_url_small || image.image_url_cropped || image.image_url || null
}
...
async function processCard(card, options, counters) {
  ...
  const imageUrl = getThumbUrl(card)
  if (!imageUrl) {
    counters.missing += 1
    return
  }
...
async function main() {
  ...
  console.log(`Total de miniaturas para verificar: ${list.length}`)
        ...
        if (
          ...
          console.log(
            `[${currentIndex + 1}/${list.length}] lote ${
              batchIndex + 1
            }/${batches.length} | skipped=${counters.skipped} downloaded=${counters.downloaded} missing=${counters.missing} failed=${counters.failed}`
          )
        }
      } catch (error) {
        counters.failed += 1
